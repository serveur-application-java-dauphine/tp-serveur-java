...
public class OrdreManagedBean implements Serializable {
	...
	/**
	 * @param idDirectionOrdre
	 *            the idDirectionOrdre to set
	 */
	...
	public List<Ordre> getPendingOrders() {
		return so.allPendingOrdres(sumb.getUtilisateur().getPortefeuille()
				.getIdPortefeuille());
	}
	...
	public List<Transaction> getDoneOrders() {
		return so.allDoneOrdres(sumb.getUtilisateur().getPortefeuille()
				.getIdPortefeuille());
	}
	...
	public Ordre getOrdre() {
		if (ordre == null) {
			...
			dir.setIdDirectionOrdre(idDirectionOrdre != 0 ? idDirectionOrdre
					: (long) 1);
	...
	public void changeSocieteListener(ValueChangeEvent event) {
		...
		ordre.getProduit().getSociete()
				.setIdSociete((Long) event.getNewValue());
		spmb.setProduits(sp.getListProductBySocieteId((Long) event
				.getNewValue()));
	}
	...
	public void passerOrdre() {
		if (ordre.getProduit().getIdProduit() == 0) {
			Utilities.addError("Vous devez s�lectionner un produit", null);
			return;
		}
		if (ordre.getTypeOrdre().getIdTypeOrder() == 3) {
			...
			if (ordre.getDirectionOrdre().getIdDirectionOrdre()
					.equals(new Long(2))
					&& ordre.getQuantite() > quantiteActif(ordre.getQuantite())) {
				Utilities.addError("Vous ne disposez que de "
						+ quantiteActif(ordre.getProduit().getIdProduit())
						+ " " + ordre.getProduit().getLibelleProduit(), null);
				return;
			}
			ordre.setQuantiteNonExecute(ordre.getQuantite());
			ordre.setPortefeuille(sumb.getUtilisateur().getPortefeuille());
	...
	private BigDecimal total() {
		...
		if (ordres != null) {
			for (Transaction t : ordres)
				total = total.add(t.getPrix().multiply(
						new BigDecimal(t.getQuantite())));
		}
		return total;
	...
	public List<Produit> getActifs() {
		if (actifs == null)
			actifs = sp.getActifs(sumb.getUtilisateur().getPortefeuille()
					.getIdPortefeuille());
		return actifs;
	}

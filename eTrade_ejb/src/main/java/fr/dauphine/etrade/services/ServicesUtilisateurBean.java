...
//	private EntityManager em;	
	...
	public Utilisateur getUtilisateurByEmail(String email) {
		Utilisateur result = null;
		Query q = (Query) em.createQuery("SELECT u FROM Utilisateur u left join u.role left join u.societe left join u.portefeuille WHERE u.email= ?",Utilisateur.class);
		q.setParameter(1, email);
		try{
			result = (Utilisateur) q.getSingleResult();
		//User not found	
